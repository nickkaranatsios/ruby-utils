...
void QuicSimpleServerStream::WriteData(StringPiece message, bool fin) {
  if (!reading_stopped()) {
    StopReading();
  }
  WriteOrBufferData(message, fin, nullptr);
}
...
void QuicSimpleServerStream::OnDataAvailable() {
  ...
  if (write_side_closed() || fin_buffered()) {
    DVLOG(1) << "write side closed or fin buffered";
    return;
  }
